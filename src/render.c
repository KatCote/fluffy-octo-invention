...
void render_connection_points(int arr_size, point* tmp_point_arr)
{
  ...
  {
    #ifdef DEV_MODE
    mvprintw(tmp_point_arr[counter].y, tmp_point_arr[counter].x, "(%d)", tmp_point_arr[counter].number);
     #else
    mvprintw(tmp_point_arr[counter].y, tmp_point_arr[counter].x, POINT_CHAR);
    #endif
  }
}
